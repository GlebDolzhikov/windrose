Meteor.methods({
    ...
    editFlt: function (id, depDirection,arrDirection) {
        Data.update({_id: id}, {
            $set: {
    ...
    fltFixToggle: function (id,isOrange) {
    ...
    migration: function () {
        Data.find({name: 'flight'}).forEach(function (flights) {
            var depDirection = flights.direction .substring(0,3);
            var arrDirection = flights.direction .substring(4,7);
            ...
            console.log(flights.direction + " was updated!")
        });
...
EditableText.registerCallbacks({
    "callSplit" : function(doc, Collection, newValue) {
        newValue = newValue.replaceAt(3, " ").toUpperCase();
        var depDirection = newValue.substring(0,3);
        var arrDirection = newValue.substring(4,7);
        Meteor.call("editFlt",doc._id, depDirection,arrDirection);
