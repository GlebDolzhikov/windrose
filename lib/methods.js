...
Meteor.methods({
    ...
    updFltLength:function(direction,id){
        if(this.userId){
            var user = Meteor.users.findOne(this.userId);
            if (user.username!='Guest') {
                var direct = Data.findOne({direction: direction.toUpperCase()});
                if (direct) {
                    var time = direct.time;
                    var dur = (time / 5) * 3;
    ...
    updAllFltLength:function(direction,time){
        ...
        if(flights){
            for(var i = 0; i<flights.length;i++){
                ...
                Data.update({_id: flights[i]._id},{$set:{width:dur}})
            }
        }
    },
    ...
    downloadExcelFile : function(id) {
        ...
        worksheet.writeToCell(0,0, skedName); // Example : writing to a cell
        worksheet.mergeCells(0,0,0,0,1); // Example : merging files
